var HostingPlan = (function () {
    ...
    function BindHostingPlanList() {
        var Success = function (result) {
            if (result != "" || result.length >= 0) {
                $('#hostingPlanlist').DataTable({
                    data: result,
                    "bFilter": true,
                    "bLengthChange": true,
                    "bDestroy": true,
                    ...
                    columns: [
                        ...
                        },
                        {
                            ...
                            "render": function (data, type, full, meta) {
                                return '<i class="fas fa-edit" id="editPlan" data-value ="' + data + '"></i>|<i class="fas fa-trash-alt" id="deletePlan" data-value ="' + data + '" ></i>';
                            }
                        }
                    ]
                });
        ...
        var url = '/ProductAdmin/Plans/GetHostingPlanList/';
    ...
    $('body').on('click', "#btnSubmit", function () {
        ...
        var hostingSize = CommonUtility.ScriptInjection($("#txtHostingSize").val()).replace(/</g, "").replace(/>/g, "");
        ...
        var hostingSizeStatus = CommonValidation.MandetoryControlValueCheck("TEXTBOX", "txtHostingSize");
        ...
        if (categoryStatus && validityStatus && planNameStatus && offerCostStatus && actualCostStatus && hostingSizeStatus) {
            if (planID != "")
                UpdatePlan(planID, category, validity, planName, offerCost, actualCost, planStatus, desc, hostingSize)
            else
                AddPlan(category, validity, planName, offerCost, actualCost, planStatus, desc, hostingSize);
        }
    });

    function AddPlan(category, validity, planName, offerCost, actualCost, planStatus, desc, hostingSize) {
        ...
        var url = '/ProductAdmin/Plans/AddHostingPlan/';
        CommonUtility.RequestAjax('POST', url, data, Success, null, null, null);
    }

    function UpdatePlan(planID, category, validity, planName, offerCost, actualCost, planStatus, desc, hostingSize) {

        var Success = function (result) {
            if (result != "" || result.length > 0) {
                CommonUtility.SucessMessagePopUp("Plan updated successfully.");
                Clear();
                BindHostingPlanList();
            }
        }
        ...
        var url = '/ProductAdmin/Plans/UpdateHostingPlan/';
        CommonUtility.RequestAjax('POST', url, data, Success, null, null, null);
    }
    //#endregion
    ...
    function DeletePlan(PlanID) {
        ...
        var url = '/ProductAdmin/Plans/DeleteHostingPlan/';
        CommonUtility.RequestAjax('POST', url, data, Success, null, null, null);
    }
    //#endregion
    ...
    $('body').on('click', "#editPlan", function () {
        ...
        var Sucess = function (result) {
            if (result != "" || result.length >= 0) {
                ...
                $("#txtHostingSize").val(result.HostingSize);
        ...
        var url = '/ProductAdmin/Plans/GetHostingPlanByID/';
    ...
    return {
        BindHostingPlanList: BindHostingPlanList,
        AddPlan: AddPlan,
        Clear: Clear,
