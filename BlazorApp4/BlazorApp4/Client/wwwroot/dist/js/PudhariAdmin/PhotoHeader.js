var PhotoHeader = (function () {
    ...
    function BindHeaderList() {
        ...
        var url = '/PudhariAdmin/PageAdmin/GetPhotoHeaderList/';
    ...
    $('body').on('click', "#btnSubmit", function () {
        ...
        var headerID = CommonUtility.ScriptInjection($("#hdnPhotoHeaderID").val()).replace(/</g, "").replace(/>/g, "");
    ...
    function AddHeader(headerName, desc, status) {
        var Success = function (result) {
            if (result != "" || result.length > 0) {
                ...
                BindHeaderList();
                if (result === 1)
                    CommonUtility.SucessMessagePopUp("Photo header saved successfully.");
                if (result === -1)
                    CommonUtility.SucessMessagePopUp("Photo header exists, could you try another name.");
            }
        }
        var data = {
        ...
        var url = '/PudhariAdmin/PageAdmin/AddPhotoHeader/';
    ...
    function UpdateHeader(headerName, desc, status, headerID) {
        var Success = function (result) {
            if (result != "" || result.length > 0) {
                Clear();
                CommonUtility.SucessMessagePopUp("Photo header updated successfully.");
                BindHeaderList();
            }
        }
        var data = {
        ...
        var url = '/PudhariAdmin/PageAdmin/UpdatePhotoHeader/';
        CommonUtility.RequestAjax('POST', url, data, Success, null, null, null);
    }
    //#endregion
    ...
    $('body').on('click', "#deleteHeader", function () {
        ...
        CommonUtility.DeleteWarningMessagePopIp("Are you sure you want to delete photo header", id);
    });
    ...
    function DeleteHeader(HeaderID) {
        ...
        var Success = function (result) {
            if (result != "" || result.length > 0) {
                ...
                CommonUtility.SucessMessagePopUp("Photo header deleted successfully.");
                BindHeaderList();
                Clear();
            }
        }
        ...
        var url = '/PudhariAdmin/PageAdmin/DeletePhotoHeader/';
        CommonUtility.RequestAjax('POST', url, data, Success, null, null, null);
    }
    //#endregion
    ...
    $('body').on('click', "#editHeader", function () {
        ...
        var BindSuccess = function (result) {
            if (result != "" || result.length >= 0) {
                ...
                $("#hdnPhotoHeaderID").val(result.HeaderID);
            }
        }
        var data = {
            HeaderID: headerID
        };
        var url = '/PudhariAdmin/PageAdmin/GetPhotoHeaderByID/';
